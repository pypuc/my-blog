...
function renderPosts(filteredPosts = posts) {
    ...
    filteredPosts.forEach((post)=>{
      ...
      <h3>${post.title}</h3>
      <p>${post.body}</p>
      <button class="delete-btn" data-id="${post.id}">\u{412}\u{438}\u{434}\u{430}\u{43B}\u{438}\u{442}\u{438}</button>
...
document.getElementById("search").addEventListener("input", (e)=>{
    ...
    const filtered = posts.filter((post)=>post.title.toLowerCase().includes(query) || post.body.toLowerCase().includes(query));
    renderPosts(filtered);
});
loadPosts();

},{}]},["7SvX3","kyksZ"], "kyksZ", "parcelRequirebe91", {})

//# sourceMappingURL=see-pictures.d29bb997.js.map
